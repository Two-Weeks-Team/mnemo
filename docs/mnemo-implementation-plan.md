...
### Feasibility changes
- **Model name**: the plan says "Gemma 4 E4B" — verified against the official Gemma 4 model card (`ai.google.dev/gemma/docs/core/model_card_4`) and the `google/gemma-4-E4B` HF repo. (The feasibility critic, whose knowledge cutoff predates Gemma 4's release, thought this was a misnamed "Gemma 3n E4B" — Gemma 4 carries the "E" effective-parameter / per-layer-embedding architecture forward from 3n.) **CLOSED (2026-05-12)**: the He Was Socrates codebase wires `LLMRegistry.gemma4_e4b_it_4bit` from `mlx-swift-lm` ≥ 3.31.3, which resolves to the HuggingFace repo **`mlx-community/gemma-4-e4b-it-4bit`** (`ModelIntegrity.modelSourceURL` = `https://huggingface.co/mlx-community/gemma-4-e4b-it-4bit`). Recorded in `Sources/MnemoEngine/Reason/FunctionCallGenerating.swift`. (Open: the plan says "128K context" / `google/gemma-4-E4B`; the He Was Socrates "Out Loud" master plan corrected an instance of "128K" to "256K" — re-check the Gemma 4 model card's context length before relying on a number, and reconcile the `mlx-community/...` vs `google/gemma-4-E4B` repo refs.)
