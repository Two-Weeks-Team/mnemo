...
After that, **Phase 4** — real macOS capture (`ScreenCaptureKit` + the screen-recording entitlement + the TCC flow + the non-dismissible mic indicator), `AVAudioEngine`+VAD+STT (push-to-capture default), clipboard (read-only), files, manual; `BlackoutPolicy` enforced — then the macOS app (Phase 5), iOS (Phase 6), the hardening/distribution pass (Phase 7). Those are platform work measured in months — see the phase table above and the plan §6/§10.
